...
const pendingItemsForQualifiedPOSecondApproversQueryV3 = `WITH timeout_config AS (
  SELECT
    CASE
...
standard_threshold AS (
  SELECT COALESCE(
    (
      SELECT second_approval_threshold
...
pos_needing_second_approval AS (
  SELECT
    ...
    COALESCE(ek.second_approval_threshold, st.second_approval_threshold, 0) AS second_approval_threshold
  FROM purchase_orders po
  LEFT JOIN expenditure_kinds ek ON po.kind = ek.id
  CROSS JOIN cfg
  CROSS JOIN standard_threshold st
  WHERE
    po.approved != ''
    AND po.rejected = ''
    ...
    AND COALESCE(ek.second_approval_threshold, st.second_approval_threshold, 0) > 0
    AND po.approval_total > COALESCE(ek.second_approval_threshold, st.second_approval_threshold, 0)
),
...
func init() {
	m.Register(func(app core.App) error {
		...
		return setPendingSecondApproverViewQuery(app, pendingItemsForQualifiedPOSecondApproversQueryV2)
	})
}

func setPendingSecondApproverViewQuery(app core.App, query string) error {
