...
func TestCleanPurchaseOrder_PersistsBlankCurrencyWhenHomeCurrencyExists(t *testing.T) {
	app := testseed.NewSeededTestApp(t)
	defer app.Cleanup()
	if err := utilities.ValidateExpenditureKindsConfig(app); err != nil {
		t.Fatalf("failed to load expenditure kinds config: %v", err)
	...
	if got := record.GetString("currency"); got != cadCurrency.Id {
		t.Fatalf("expected blank PO currency to persist home currency %s, got %q", cadCurrency.Id, got)
	}
}

func TestCleanPurchaseOrder_ChildBlankCurrencyInheritsParentBeforeHomeFallback(t *testing.T) {
	app := testseed.NewSeededTestApp(t)
	defer app.Cleanup()
	if err := utilities.ValidateExpenditureKindsConfig(app); err != nil {
		t.Fatalf("failed to load expenditure kinds config: %v", err)
	...
	if err := CleanPurchaseOrder(app, record); err != nil {
		t.Fatalf("expected CleanPurchaseOrder to succeed for child PO, got %v", err)
	}
	if got := record.GetString("currency"); got != usdCurrency.Id {
		t.Fatalf("expected child PO to inherit parent currency %s, got %q", usdCurrency.Id, got)
	}
}
