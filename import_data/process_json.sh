...
# - Sorts jobNumbers array within each parent object
...
jq 'sort_by(.uid) | map(del(.rejected, .submitted, .exported, .locked, .approved, .rejectionReason, .id) | .payrollId |= tostring | .entries |= (map(del(.weekEnding, .id) | to_entries | sort_by(.key) | from_entries) | sort_by(.timetype, .job, .client, .division, .workDescription, .date)) | .jobNumbers |= sort | to_entries | sort_by(.key) | from_entries)' "$input_file" > "$temp_file"
