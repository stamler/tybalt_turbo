...
<header class="w-screen h-10 bg-neutral-700 text-white px-3 flex justify-between">
  ...
  <!-- <template x-if="loggedIn"> -->
    ...
    <span class="flex items-center h-full gap-2">
      {#if authStoreValue !== null && authStoreValue?.isValid }
        <span>{ authStoreValue?.model?.email }</span>
        <button class="bg-blue-500 hover:bg-blue-700 text-white text-sm py-1 px-2 rounded" on:click={ authStore.logout }>
