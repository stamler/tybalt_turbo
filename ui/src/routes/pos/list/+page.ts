...
export const load: PageLoad = async ({ depends }) => {
  ...
  try {
    ...
    items = await pb.collection("purchase_orders").getFullList<PurchaseOrdersResponse>({
      ...
      expand:
        "uid.profiles_via_uid,approver.profiles_via_uid,division,vendor,job,job.client,rejector.profiles_via_uid,category,second_approver.profiles_via_uid,second_approver_claim",
      sort: "-date",
    });
    return {
      items,
