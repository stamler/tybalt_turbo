...
3. PO is not rejected (`rejected = ''`)
4. PO status is still `Unapproved`
5. PO is past Stage-2 timeout window (`approved + T`)
6. PO is dual-required for its kind (`approval_total > second_approval_threshold` and threshold > 0)
7. User is an active `po_approver`
